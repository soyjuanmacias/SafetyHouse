...
module.exports = {
  ...
  listUser: function(req, res) {
    contactModel.find(function(err, complaints) {
      if (err) {
  ...
  create: function(req, res) {
    ...
    const complaint = new contactModel({
      title,
      description,
      img_url,
      lat,
  ...
  update: function(req, res) {
    ...
    contactModel.findOne({
      ...
      //
      // contactModel.save({_id: props.id, })
  ...
  remove: function(req, res) {
    const id = req.params.id;
    contactModel.findByIdAndRemove(id, function(err, complaint) {
      if (err) {
        return res.status(500).json({
